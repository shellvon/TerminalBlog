...
TermlibInvaders = {
  ...
  waveStart: function() {
        ...
        for (var i=0; i<inv.blockpos.length; i++) {
            ...
            inv.block[x]=true;
            TermlibInvaders.drawSprite(this, inv.blockY, x, 'H', inv.shipColor);
        }
        inv.bomb=new Array();
        inv.shipX=inv.shipCenter;
        TermlibInvaders.drawScoreBG.apply(this);
        TermlibInvaders.displayScore.apply(this);
        TermlibInvaders.drawSprite(this, inv.shipY, inv.shipX, TermlibInvaders.sprites[4], inv.shipColor);
  ...
  mainLoop: function() {
        if (this.env) {
            ...
            if (inv.population==0) {
                ...
                this.lock=true;
                inv.phase=(inv.over)? 5:4;
                TermlibInvaders.gameOver.apply(this);
            }
            else {
                inv.timer=setTimeout(function() { TermlibInvaders.mainRepeat.apply(inv.termref); }, 1);
            }
        }},
  mainRepeat: function() {
        if (this.env) {
            ...
            clearTimeout(inv.timer);
            var now=new Date();
            var delay=Math.max(1, inv.delay-(now.getTime()-inv.enterTime));
  ...
  gameOver: function() {
        ...
        if (inv.phase>=TermlibInvaders.sprites.length) {
            ...
            inv.timer=setTimeout(function() { TermlibInvaders.gameOver.apply(inv.termref); }, inv.delay*2);
        }
    },
  keyHandler: function() {
        var inv=this.env.invaders;
